const CACHE='muretto-v8-visual-suite-20260713';
const CORE=['./','./index.html','./assets/app-v8.css?v=20260713-visual4','./assets/app-v8.js?v=20260713-visual4','./manifest.webmanifest','./icons/icon-192.png','./icons/icon-512.png'];
...
async function networkFirst(request){const cache=await caches.open(CACHE);try{const response=await fetch(request,{cache:'no-store'});if(response&&response.ok)cache.put(request,response.clone());return response;}catch(_e){return (await cache.match(request))||(request.mode==='navigate'?await cache.match('./index.html'):Response.error());}}
